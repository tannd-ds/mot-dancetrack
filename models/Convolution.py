...
class Conv2dPredictor(BasePositionPredictor):
    def __init__(self, config, input_dim=8, output_dim=4):
    ...
    def forward(self, conditions):
        ...
        delta_bbox = self.fc2(x) # (batch_size, output_dim)
        return delta_bbox
    
class Huy_Conv2dPredictor(BasePositionPredictor):
    def __init__(self, config, input_dim=8):
        super(Huy_Conv2dPredictor, self).__init__(config)
        self.input_dim = input_dim
        self.config = config
        self.conv1 = nn.Conv2d(in_channels=1,
                               out_channels=32,
        ...
        self.intermediate_fc = nn.Linear(self.input_dim * (self.config['interval']-4) * 128, 64)

        if self.config.num_classifiers == 1:
            self.output_dim = 8
            self.predictor = nn.Linear(64, self.output_dim)
        else: 
            self.output_dim = 4
            self.predictors = nn.Sequential(OrderedDict([
                ('delta_predictor', nn.Linear(64, self.output_dim)),
                ('position_predictor', nn.Linear(64, self.output_dim))
            ]))
    ...
    def forward(self, conditions):
        ...
        # Intermaediate FC layer
        intermediate_feature = torch.relu(self.intermediate_fc(x)) # (batch_size, 64)

        if self.config.num_classifiers == 1:
            delta_bbox = self.predictor(intermediate_feature) # (batch_size, output_dim)
            return delta_bbox
        
        elif self.config.num_classifiers == 2:
            delta_bbox = self.predictors.delta_predictor(intermediate_feature)
            position = self.predictors.position_predictor(intermediate_feature)
            return (delta_bbox, position)
